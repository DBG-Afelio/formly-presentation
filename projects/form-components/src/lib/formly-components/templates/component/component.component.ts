...
export class ComponentComponent extends FieldType implements DoCheck, OnDestroy {
    ...
    ngDoCheck(): void {
        const currentComponentPropertiesHash = this.to['componentProperties'] ? JSON.stringify(this.to['componentProperties']) : '';
        if (
            !this.to['component'] ||
    ...
    private initComponent() {
        ...
        if (this.to['listeners']) {
            const componentInstance = component.instance;
            this['listeners'] = Object.keys(this.to['listeners']).reduce((acc: any[], listener) => {
                if (listener in componentInstance && componentInstance[listener] instanceof EventEmitter) {
                    return [...acc, componentInstance[listener].subscribe((event: any) => {
