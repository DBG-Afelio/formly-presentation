...
export class RepeatFormlyGroupComponent extends FieldArrayType implements OnInit, OnDestroy, DoCheck {
    ...
    public add(i?: number, initialModel?: any, markAsDirty?: { markAsDirty: boolean }): void {
        if (initialModel || !this.to.defaultModel) {
            super.add(i, initialModel, markAsDirty);
        } else {
            super.add(i, this.to.defaultModel, markAsDirty);
        }
    }
    ...
    public duplicateItem(index: number, minMax: MinMaxInterface) {
        if (this.canAddGroup(minMax)) {
            ...
            let modelToDuplicate = cloneDeep(this.field.fieldGroup[index].model);
            if (this.to.onDuplicate) {
                modelToDuplicate = this.to.onDuplicate(modelToDuplicate);
            }
