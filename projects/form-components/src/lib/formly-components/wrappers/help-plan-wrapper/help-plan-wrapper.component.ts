...
export class HelpPlanWrapperComponent extends FieldWrapper implements OnInit, DoCheck, OnDestroy {
    ...
    ngDoCheck() {
        const to = this.to as FormlyTemplateOptions;
        if (this.to['fields'] && this.to['refresh']) {
            const fieldsNotNull = this.to['fields'].reduce((acc: any[], current: any) => {
    ...
    private reduceMap(dico: any, label: string, setValueFn?: Function): ListFieldToDisplay {
        const reduced =  {
            type: 'list',
            label,
            values: Object.keys(dico).reduce((acc: any, current: any) => {
                if (dico[current]) {
